    ...
    public class RealGun : VRTK_InteractableObject
    {
        ...
        public override void StartUsing(GameObject currentUsingObject)
        {
            ...
            if (safetySwitch.safetyOff)
            {
                slide.Fire();
                FireBullet();
                VRTK_SharedMethods.TriggerHapticPulse(VRTK_DeviceFinder.GetControllerIndex(controllerEvents.GetTrackedHand()), 0.63f, 0.2f, 0.01f);
            }
            else
            {
                VRTK_SharedMethods.TriggerHapticPulse(VRTK_DeviceFinder.GetControllerIndex(controllerEvents.GetTrackedHand()), 0.08f, 0.1f, 0.01f);
            }
        }
